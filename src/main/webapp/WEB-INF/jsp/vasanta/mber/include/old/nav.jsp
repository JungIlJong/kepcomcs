        ...
        <c:forEach var="depth1" items="${menuItems.childList}" varStatus="depth1Status">
            ...
            <c:if test="${fn:contains(fn:split(depth1Url, '/')[1], fn:split(requestUri, '/')[1])}">
                <c:forEach var="depth2" items="${depth1.childList}" varStatus="depth2Status">
                    ...
                    <li class="toggle <c:if test="${fn:contains(fn:split(depth2Url, '/')[2], fn:split(requestUri, '/')[2])}">show</c:if>">
                        <c:choose>
                            <c:when test="${fn:length(depth2.childList) gt 0}">
                                <a href="javascript:void(0);" class="asideBtn toggleBtn">
                                    <span><c:out value="${depth2.menuNm}"/></span>
                                    <img class="arrow" src="/mber/assets/icon/Basic_Icon/Accordion_arrow.svg" alt="">
                                </a>
                            </c:when>
                            <c:otherwise>
                                <a <c:if test="${depth2.menuTy eq 'LINK'}">target="_blank"</c:if> href="<c:url value="${depth2Url}"/>" class="asideBtn toggleBtn">
                                    <span><c:out value="${depth2.menuNm}"/></span>
                                </a>
                            </c:otherwise>
                        </c:choose>
                        <div class="asideSubCon">
                            <c:forEach var="depth3" items="${depth2.childList}" varStatus="depth3Status">
                                ...
                                <a <c:if test="${depth3.menuTy eq 'LINK'}">target="_blank"</c:if> href="<c:url value="${depth3Url}"/>" class="<c:if test="${fn:split(depth3Url, '/')[4] eq fn:split(requestUri, '/')[4]}">active</c:if>"><c:out value="${depth3.menuNm}"/></a>
                            </c:forEach>
                        </div>
                    </li>
                </c:forEach>
