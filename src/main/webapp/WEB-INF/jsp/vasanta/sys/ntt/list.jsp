...
<head>
    ...
    <script type="text/javascript">
        ...
        let setSelect = function (data) {
            ...
            let bbsId = getUrlParameter('bbsId');
            let select = document.getElementById('bbsId');
            let start = 0;
            data.forEach(function (element, index) {
                bbsId === element.bbsId ? start = index + 1 : '';
                let option = `<option value="<c:out value="${'${element.bbsId}'}"/>"><c:out value="${'${element.bbsNm}'}"/></option>`;
                select.insertAdjacentHTML('beforeend', option);
            })
            $('#bbsId').prop('selectedIndex', start).find(`option:eq(${'${start}}'})`).prop("selected", true);
        ...
        let checkAuth = function (data) {
            if (data.authorList.includes('SAVE')) {
                document.querySelectorAll('.save').forEach((element, index) => {
                    element.href = 'save';
                });
            }
        ...
        let successDelete = function (data) {
            customAlert({
                ...
                showCancel: false,
                onConfirm: function() {
                    new DataTable('.datatables-ntts').draw();
                }
            });
        }
                                    ...
                                    <div class="row mx-3 my-0 justify-content-between">
                                        <div>
                                            <div class="row me-4 mb-4 col-12 justify-content-between">
                                                ...
                                                <div class="col-6">
                                                    <div class="form-control-validation fv-plugins-icon-container">
                                                        <label class="form-label" for="bbsId"><spring:message
                                                                code="bbs.bbsNm"/></label>
                                                        <select class="form-control form-select" id="bbsId"
                                                                name="bbsId">
                                                            <option value=""><spring:message code="table.all"/></option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
...
<script src="/static/pages/sys/js/ntt/list.js"></script>
<!-- script -->
<script type="text/javascript">
    $(function () {
        let dt_ntt = new DataTable('.datatables-ntts');
        // Refilter the table
        document.querySelectorAll('#stDt, #endDt, #bbsId').forEach((el) => {
            el.addEventListener('change', () => dt_ntt.draw());
        });

        if (location.href.includes('ntt')) {
            // document.getElementById('bbsIdListTitle').style.removeProperty('display');
            // document.getElementById('bbsIdList').style.removeProperty('display');
            Ajax.get('/api/sys/bbs/all', setSelect, fail);
        } else {
            let depth = getUrlDepth();
            document.getElementById('bbsId').insertAdjacentHTML('afterbegin', `<option value="<c:out value="${'${depth[depth.length - 2]}'}"/>"></option>`);
            $('#bbsId').prop('selectedIndex', 0).find("option:eq(0)").prop("selected", true);
            const bbsId = $('#bbsId').val();
            // dt_ntt.draw();
            Ajax.get(dataToQueryString('/api/sys/bbs/info', {bbsId}), checkAuth, fail);
        }

        dt_ntt.draw();
    });

    function deleteNtt(nttId) {
        if (confirm('<spring:message code="confirm.common.delete"/>')) {
            const data = {
                "nttId": nttId
            };
            Ajax.post('/api/sys/ntt/delete', successDelete, fail, {data: JSON.stringify(data)});
        }
    }
