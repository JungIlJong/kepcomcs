...
<script>
    ...
    let successNtfList = function (data) {
        var ntfList = document.getElementById("ntfList");

        if (data.length > 0) {
            document.getElementById("ntfExist").insertAdjacentHTML('beforeend', '<span class="badge rounded-pill bg-danger badge-dot badge-notifications border"></span>');
        }
        for (var i = 0; i < data.length; i++) {
            let templete = document.getElementById("ntfElem");
            let ntfElemTxt = document.importNode(templete.content, true);

            ntfElemTxt.querySelector("input.ntfId").value = data[i].ntfId;
            ntfElemTxt.querySelector("h6.ntfTitle").textContent = data[i].ntfTitle;
            ntfElemTxt.querySelector("small.ntfContent").textContent = data[i].ntfContent;
            ntfElemTxt.querySelector("small.ntfRegistDt").textContent = data[i].ntfRegistDt;
            if (data[i].ntfRedirectUrl) {
                var ntfRedirectUrl = data[i].ntfRedirectUrl;
                ntfElemTxt.querySelector(".ntfRedirectUrl").addEventListener("click", function (e) {
                    goPage(ntfRedirectUrl);
                })
            }

            if (data[i].ntfReadYn == "Y") {
                ntfElemTxt.querySelector(".dropdown-notifications-item").classList.toggle('marked-as-read');
            }

            ntfList.appendChild(ntfElemTxt);
        }
        ...
        if (notificationMarkAsReadList) {
            notificationMarkAsReadList.forEach(item => {
                item.addEventListener('click', event => {
                    ...
                    var ntfId = item.parentElement.parentElement.querySelector('input.ntfId').value;
                    var resultMap =  {ntfId : ntfId, ntfReadYn:
                            item.closest('.dropdown-notifications-item').classList.contains('marked-as-read') ? "Y" : "N"};

                    Ajax.post('/api/sys/notification/update', successRead, failRead, {data: JSON.stringify(resultMap)});
                });
            });
        }
        ...
        notificationArchiveMessageList.forEach(item => {
            item.addEventListener('click', event => {
                var ntfId = item.parentElement.parentElement.querySelector('input.ntfId').value;
                Ajax.post(dataToQueryString('/api/sys/notification/delete', {ntfId : ntfId}), successRead, failRead);
                item.closest('.dropdown-notifications-item').remove();
            });
        });
    }

    let failNtfList = function (xhr, status, error) {
        if (isEmpty(xhr.responseJSON)) {
            customAlert({
                title: '<spring:message code="common.system.info"/>',
                content: '알림 목록 조회에 실패했습니다.',
                showCancel: false,
                onConfirm: function() {
                }
            });
    ...
    $(function () {
        Ajax.get(dataToQueryString('/api/sys/notification/list/byMbrId', {}), successNtfList, failNtfList);
    })
</script>
