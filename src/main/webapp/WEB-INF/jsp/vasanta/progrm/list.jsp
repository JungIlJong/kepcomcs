<%@ page contentType="text/html;charset=UTF-8" language="java" pageEncoding="UTF-8" %>
<%@ include file="/WEB-INF/jsp/vasanta/cmmn/taglib.jsp" %>
<html lang="ko" data-theme="light" data-skin="bordered" dir="ltr" data-bs-theme="light" data-template="vertical-menu-template" data-assets-path="/vendor/sneat/">
<head>
    <%@ include file="/WEB-INF/jsp/vasanta/sys/include/head.jsp" %>
    <title>프로그램관리</title>
    <script type="text/javascript">
        <%-- 삭제 성공 시 Callback 함수 --%>
        let successDelete = function (data) {
            alert('<spring:message code="success.common.delete"/>');
            new DataTable('.datatables-programs').draw();
        }

        <%-- Ajax 요청실패 시 Callback 함수 --%>
        let fail = function (xhr, status, error) {
            alert('<spring:message code="fail.common.msg"/>');
            console.log(xhr + ", " + status + " : " + error)
        }
    </script>
</head>
<body>
<div class="layout-wrapper layout-content-navbar">
    <div class="layout-container">
        <!-- Menu -->
        <%@ include file="/WEB-INF/jsp/vasanta/sys/include/sneat-menu.jsp" %>
        <!-- / Menu -->

        <!-- Layout page -->
        <div class="layout-page">
            <!-- Navbar -->
            <%@ include file="/WEB-INF/jsp/vasanta/sys/include/sneat-nav.jsp" %>
            <!-- / Navbar -->

            <!-- Content wrapper -->
            <div class="content-wrapper">
                <!-- Content -->
                <div class="container-xxl flex-grow-1 container-p-y">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="mb-0"><c:out value="${menuTitle}"/></h5>
                        </div>
                        <div class="card-body row">
                            <div class="justify-content-between dt-layout-table">
                                <div class="justify-content-between align-items-center dt-layout-full table-responsive">
                                    <table class="datatables-programs table border-top dataTable dtr-column"
                                           id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<script src="/static/pages/sys/js/progrm/list.js"></script>
<script type="text/javascript">
    $(function () {
        let dt_program = new DataTable('.datatables-programs');
        dt_program.draw();
    });

    function deleteProgram(progrmId) {
        if (confirm('<spring:message code="confirm.common.delete"/>')) {
            const data = {
                "progrmId": progrmId.toString()
            };
            Ajax.post('/api/sys/progrm/delete', successDelete, fail, {data: JSON.stringify(data)});
        }
    }
</script>
</body>

</html>
