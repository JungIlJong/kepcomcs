...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_banner_table = document.querySelector('.datatables-banners'),
  ...
  // Users datatable
  if (dt_banner_table) {
    const dt_banner = new DataTable(dt_banner_table, {
      ajax: {
        url: '/api/sys/banner/list',
        type: "GET",
        dataSrc: 'data',
        data: function (d) {
          d.minDate = $('#stDt').val();
          d.maxDate = $('#endDt').val();
          d.reflctAt = $("#reflctAt").val();
          d.bannerSeCode = $("#bannerSeCode").val();
          return d;
        }
      },
      serverSide: true,
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            let now = new Date();
            if(data.reflctAt === 'Y' && ( now >= new Date(data.ntceBgnde) && now <= new Date(data.ntceEndde))) {
              return '<span class="badge text-bg-primary">활성</span>';
            } else if(data.reflctAt === 'Y' && now > new Date(data.ntceEndde)) {
          ...
          render: (data, type, full, meta) => {
            return `
              <div class="d-flex gap-1 justify-content-center">
                <button onclick="location.href='/sys/operate/main/banner/banner/detail?bannerId=${data}'" class="btn btn-sm btn-label-secondary">
                  <i class="icon-base bx bx-pencil icon-xs"></i>수정
                </button>
                <button onclick="deleteBanner('${data}')" class="btn btn-sm btn-label-danger">
    ...
    function bindDeleteEvent() {
      const userListTable = document.querySelector('.datatables-banners');
