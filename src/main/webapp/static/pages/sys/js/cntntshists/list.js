...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_cntntshists_table = document.querySelector('.datatables-cntntshists'),
  ...
  // Users datatable
  if (dt_cntntshists_table) {
    const dt_cntntshists = new DataTable(dt_cntntshists_table, {
      ajax: {
        url: '/api/sys/cntntshist/list',
        type: "GET",
        dataSrc: 'data',
        data: function (d) {
          d.minDate = $('#stDt').val();
      ...
      columnDefs: [
        ...
        },
        {
          ...
          searchable: false,
          render: function (data, type, full, meta) {
            return data === 'SAVE' ? '등록' : (data === 'UPDATE' ? '수정' : '삭제')
          }
        },
        {
          // Plans
    ...
    function deleteRecord(event) {
      ...
      if (row) {
        dt_cntntshists.row(row).remove().draw();
      }
    }

    function bindDeleteEvent() {
      const userListTable = document.querySelector('.datatables-cntntshists');
