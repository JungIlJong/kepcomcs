...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_cntnt_table = document.querySelector('.datatables-cntnts'),
  ...
  // Users datatable
  if (dt_cntnt_table) {
    const dt_cntnt = new DataTable(dt_cntnt_table, {
      ajax: {
        url: '/api/sys/cntnts/list',
        type: "GET",
        dataSrc: 'data',
        data: function (d) {
          d.minDate = $('#stDt').val();
          ...
          render: (data, type, full, meta) => {
            return `
              <div class="d-flex gap-1 justify-content-center">
                <button onclick="location.href='detail?cntntsId=${data}'" class="btn btn-sm btn-label-secondary">
                  <i class="icon-base bx bx-pencil icon-xs"></i>수정
                </button>
                <button onclick="deleteCntnt('${data}')" class="btn btn-sm btn-label-danger">
    ...
    function deleteRecord(event) {
      ...
      if (row) {
        dt_cntnt.row(row).remove().draw();
      }
    }

    function bindDeleteEvent() {
      const userListTable = document.querySelector('.datatables-cntnts');
