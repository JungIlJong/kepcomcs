...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_authrt_table = document.querySelector('.datatables-authrts'),
  ...
  // Users datatable
  if (dt_authrt_table) {
    const dt_banner = new DataTable(dt_authrt_table, {
      ajax: {
        url: '/api/sys/authrt/list',
        type: "GET",
        dataSrc: 'data',
        data: function (d) {
          return d;
          ...
          render: (data, type, full, meta) => {
            return `
              <div class="d-flex gap-1 justify-content-center">
                <button onclick="location.href='detail?authrtId=${data}'" class="btn btn-sm btn-label-secondary">
                  <i class="icon-base bx bx-pencil icon-xs"></i>수정
                </button>              
                <button onclick="deleteAuthrt('${data}')" class="btn btn-sm btn-label-danger">
    ...
    function bindDeleteEvent() {
      const userListTable = document.querySelector('.datatables-authrts');
