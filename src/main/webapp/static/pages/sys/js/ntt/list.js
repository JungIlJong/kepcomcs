...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_ntt_table = document.querySelector('.datatables-ntts'),
  ...
  // Users datatable
  if (dt_ntt_table) {
    const dt_ntt = new DataTable(dt_ntt_table, {
      ajax: {
        url: '/api/sys/ntt/list',
        type: "GET",
        dataSrc: 'data',
        data: function (d) {
          d.minDate = $('#stDt').val();
          ...
          render: function (data, type, full, meta) {
            if(data.bbsTyCode == 'BBS_QNA') {
              if(data.nttStatus == 0) {
                ...
                // node += `<td class="tblBodyItem fix200">
                //         <div class="buttonArea center">
                //           <a href="reply?reply=Y&nttId=${'${element.nttId}'}" class="btn sml line disabled"><spring:message code="ntt.answer.complete" /></a>
                //         </div>
                //       </td>`
              }
            } else {
              return `<span class="badge text-bg-danger">미해당</td>`
            }
          }
        },
        {
    ...
    function deleteRecord(event) {
      ...
      if (row) {
        dt_ntt.row(row).remove().draw();
      }
    }

    function bindDeleteEvent() {
      const userListTable = document.querySelector('.datatables-ntts');
